...
class Feature(object):
    def __init__(self, example_id, doc_start, question_len, tokenized_to_original_index, input_tokens, input_ids,
                 input_mask, segment_ids, bert_start_position=None, bert_end_position=None, label=None, passage=None,
                 origin_start=None, origin_end=None, token_start=None, token_end=None, doc_end=None, doc_id=None,
                 bert_doc_end=None, bert_answer_span=None, cls_logit=None, best_scores=None, result=None,
                 new_score=None, doc_length=None, search_pred=None):
    ...
    while i < len(selected_sentenses):
        if len(selected_sentenses[i]['tokens']) + len(doc_tokens) <= max_doc_length:
            ...
            for sentense_start in range(0, len(selected_sentenses[i]['tokens']), max_doc_length - 100):
                sentense_end = min(sentense_start + max_doc_length - 1,
                                   len(selected_sentenses[i]['tokens']) - 1)  # overlap
...
class Result(object):
    ...
    def update(
        ...
        indices = [[(int(start), int(end)) for start in start_index[i] for end in end_index[i]] for i in
                   range(start_index.size(0))]
        logits = torch.Tensor([[(start_preds[i][d[0]] + end_preds[i][d[1]] - cls_logits[i]) if self.is_valid_index(
            examples[i], d) else -1e3 for d in indices[i]] for i in range(cls_logits.size(0))])
    ...
    def generate_predictions(self):
        ...
        for example_id in self.best_scores.keys():
            ...
            if not self.is_testing:
                answers.append({'example_id': example_id, 'pred': answer,
                                'label': example.passage[int(example.origin_start):int(example.origin_end) + 1]})
            else:
    ...
    def score(self):
        ...
        if not self.is_testing:
            ...
            rouge = Rouge()
            # 修改： refs和hyps调换位置
            scores = rouge.get_scores(refs=refs, hyps=hyps, avg=True)
